...
(function () {
    ...
    function fixPaths(container, rootPath) {
        if (!container) return;

        // Ensure rootPath ends with / if it's not empty, or is empty if it is ./
        // Standardize: rootPath should be relative prefix like "../" or "../../" or "" or "./"

        container.querySelectorAll('a[data-base-path]').forEach(link => {
            const basePath = link.getAttribute('data-base-path');
            if (basePath) {
                // If the link is to a root file
                if (basePath === 'index.html' || basePath === 'where_are_we.html') {
                    // If we are at root (rootPath is empty or ./), just use filename
                    if (!rootPath || rootPath === './') {
                        link.href = basePath;
                    } else {
                        link.href = rootPath + basePath;
                    }
                } else {
                    // It's a subfolder path like 'myhealth/seasonal.html'
                    // If we are at root, correct path is 'myhealth/seasonal.html'
                    if (!rootPath || rootPath === './') {
                        link.href = basePath;
                    } else {
                        link.href = rootPath + basePath;
        ...
        // Fallback for links without data-base-path
        container.querySelectorAll('a[href^="/"]').forEach(link => {
            if (!link.hasAttribute('data-base-path')) {
    ...
    function loadComponents(rootPath) {
        ...
        const headerPath = (rootPath === './' ? '' : rootPath) + 'components/header.html';
        const footerPath = (rootPath === './' ? '' : rootPath) + 'components/footer.html';
        ...
        fetch(headerPath)
            ...
            .then(html => {
                ...
                if (headerContainer) {
                    ...
                    fixPaths(headerContainer, rootPath);

                    // Dispatch event that header is loaded (for script.js if needed)
        ...
        fetch(footerPath)
            ...
            .then(html => {
                ...
                if (footerContainer) {
                    ...
                    fixPaths(footerContainer, rootPath);
                }
            })
